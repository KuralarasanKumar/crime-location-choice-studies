...
OUTPUT_MAIN    <- "20250704_comprehensive_dataset_with_elicit_streamlined_v2.csv"
OUTPUT_MATCH   <- "20250704_elicit_matching_report_streamlined_v2.csv"
OUTPUT_RAW     <- "20250704_elicit_extracted_data_streamlined_v2.csv"
...
for (i in seq_len(nrow(match_dt))) {
  ...
  if (!is.na(el_idx)) {
    ...
    if (nrow(e) == 1) {
      ...
      for (nm in names(e_list)) std[[nm]][i] <- e_list[[nm]]
    }
  }
}
