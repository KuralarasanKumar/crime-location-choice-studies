# Target Studies Analysis - Fixed Version
...
Write-Host "CHECKING TARGET STUDIES..." -ForegroundColor Yellow

# Get relevant PDF files
$allFiles = Get-ChildItem -Path ".\PDFs" -Filter "*.pdf" | Where-Object { 
    $_.Name -ne "ToDelete"
}

Write-Host "Total PDF files found: $($allFiles.Count)"
...
foreach ($study in $targetStudies) {
    ...
    if ($found) {
        ...
        Write-Host "FOUND: $($study.SNo) - $matchedFile" -ForegroundColor Green
    } else {
        $stillMissing += $study.SNo
        Write-Host "MISSING: $($study.SNo)" -ForegroundColor Red
    }
}
...
Write-Host "MISSING: $($stillMissing.Count) studies" -ForegroundColor Red
Write-Host "TOTAL TARGET: $($targetStudies.Count) studies" -ForegroundColor Cyan
$completion = [Math]::Round(($foundStudies.Count / $targetStudies.Count) * 100, 1)
Write-Host "COMPLETION: $completion%" -ForegroundColor Magenta

if ($foundStudies.Count -gt 28) {
    Write-Host "EXCELLENT PROGRESS! New studies found!" -ForegroundColor Green
}
...
if ($irrelevantFiles.Count -gt 0) {
    Write-Host ""
    Write-Host "IRRELEVANT FILES TO CLEAN UP:" -ForegroundColor Yellow
    Write-Host "Found $($irrelevantFiles.Count) irrelevant files"
    
    $response = Read-Host "Move irrelevant files to ToDelete folder? (y/N)"
