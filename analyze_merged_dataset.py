...
def main():
    ...
    journal_counts = df[df['Journal'].notna() & (df['Journal'] != '')]['Journal'].value_counts()
    ...
    print(f"Perfect title matches: {(df['Match_Score'] == 1.0).sum()}/{len(df)} ({(df['Match_Score'] == 1.0).sum()/len(df)*100:.1f}%)")
    print(f"High confidence matches (≥0.8): {(df['Match_Score'] >= 0.8).sum()}/{len(df)} ({(df['Match_Score'] >= 0.8).sum()/len(df)*100:.1f}%)")
    print(f"Complete bibliographic info: {((df['DOI'].notna() & (df['DOI'] != '')) & (df['Journal'].notna() & (df['Journal'] != ''))).sum()}/{len(df)} studies")
    ...
    print(f"  • Publication years spanning {year_counts.index.min()}-{year_counts.index.max()}")
    print(f"{'='*60}")
