...
def main():
    ...
    try:
        ...
        try:
            biblio_df = pd.read_csv(
                '20250117_Unique_retrieved_articles.csv', encoding='latin-1')
        except UnicodeDecodeError:
            print("  Trying with ISO-8859-1 encoding...")
            biblio_df = pd.read_csv(
                '20250117_Unique_retrieved_articles.csv', encoding='iso-8859-1')
    ...
    for idx, row in main_df.iterrows():
        ...
        if best_idx is not None:
            ...
            main_df.at[idx, 'DOI'] = matched_row.get(
                'doi', '') if pd.notna(matched_row.get('doi', '')) else ''
            main_df.at[idx, 'ISSN'] = matched_row.get(
                'issn', '') if pd.notna(matched_row.get('issn', '')) else ''
            main_df.at[idx, 'Journal'] = matched_row.get(
                'journal', '') if pd.notna(matched_row.get('journal', '')) else ''
            main_df.at[idx, 'Volume'] = matched_row.get(
                'volume', '') if pd.notna(matched_row.get('volume', '')) else ''
            main_df.at[idx, 'Issue'] = matched_row.get(
                'issue', '') if pd.notna(matched_row.get('issue', '')) else ''
    ...
    print(
        f"  Studies with DOI: {has_doi}/{len(main_df)} ({has_doi/len(main_df)*100:.1f}%)")
    print(
        f"  Studies with Journal: {has_journal}/{len(main_df)} ({has_journal/len(main_df)*100:.1f}%)")
    print(
        f"  Studies with ISSN: {has_issn}/{len(main_df)} ({has_issn/len(main_df)*100:.1f}%)")
    ...
    low_confidence = main_df[(main_df['Match_Score'] > 0)
                             & (main_df['Match_Score'] < 0.8)]
