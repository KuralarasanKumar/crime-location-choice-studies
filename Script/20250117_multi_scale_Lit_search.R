...
scopus <- readr::read_csv(here::here("20250117_Analysis & Results/results_litsearch/scopus.csv"), col_types = readr::cols(.default = readr::col_character())) |>
...
google_scholar <- readr::read_csv(here::here("20250117_Analysis & Results/results_litsearch/google_scholar.csv"), col_types = readr::cols(.default = readr::col_character()))|>
  #convert the names to lowercase and replace space with "_"
  dplyr::rename_all(~stringr::str_replace_all(tolower(.), " ", "_")) |>
  # convert all the characters to lowercase 
  dplyr::mutate( dplyr::across(where(is.character), tolower))|>
...
WOS <- readxl::read_excel(here::here("20250117_Analysis & Results/results_litsearch/WOS.xls"), col_types = "text")|>
  #convert the names to lowercase and replace space with "_"
  dplyr::rename_all(~stringr::str_replace_all(tolower(.), " ", "_"))|>
  # convert all the characters to lowercase 
  dplyr::mutate( dplyr::across(where(is.character), tolower))|>
...
# read proquest data 
pro_quest <- readr::read_csv(here::here("20250117_Analysis & Results/results_litsearch/ProQuest.CSV"), col_types = readr::cols(.default = readr::col_character())) |>
  # Convert the names to lowercase and replace space with "_"
  dplyr::rename_all(~stringr::str_replace_all(tolower(.), " ", "_")) |>
  # Convert all the characters to lowercase
  dplyr::mutate(dplyr::across(where(is.character), tolower)) |>
...
data_unique <- data_unique |> 
  dplyr:: select(source_type, source, year, title, abstract, 
                 journal, volume, issue, start_page, end_page,
                 source_type, source, 
                 doi, issn) |>
  dplyr::mutate(`select_reject_by (Wim/Christophe)` = " ", 
