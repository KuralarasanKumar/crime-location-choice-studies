...
OUTPUT_MAIN    <- "20250704_comprehensive_dataset_with_elicit.csv"
OUTPUT_MATCH   <- "20250704_elicit_matching_report.csv"
OUTPUT_RAW     <- "20250704_elicit_extracted_data.csv"
...
extract_field <- function(txt, header) {
  if (is.na(txt)) return("")
  pattern <- str_glue("\\*\\*{str_replace_all(header, '([\\^$.*+?()\\[\\]{}|\\\\])', '\\\\1')}:\\*\\*\\s*(.*?)(?=\\n\\*\\*|$)")
  m <- str_match(txt, regex(pattern, ignore_case = TRUE, dotall = TRUE))
  ifelse(is.na(m[2]), "", m[2] |> str_replace_all("\\[.*?\\]", "") |> str_squish())
}
...
extract_variable_counts <- function(core) {
  ...
  for (nm in names(patterns)) {
    val <- str_match(block,
                     regex(str_glue("\\*\\*{patterns[[nm]]}:\\*\\*\\s*(\\d+)"),
                           ignore_case=TRUE))[2]
...
country_override <- function(row, std_title) {
  ...
  if (length(code)) {
    ...
    row$Study_Area_Description <-
      str_glue("Part of cross-national comparison focusing on {row$Country}. ",
               row$Study_Area_Description %||% "")
  }
  row
}
...
for (i in seq_len(nrow(match_dt))) {
  ...
  if (!is.na(el_idx)) {
    ...
    if (nrow(e) == 1) {
      e <- country_override(as.list(e), match_dt$Std_Title[i])
      e <- e[names(e) != "Elicit_Index" & names(e) != "Title_Original_Elicit"]
      setnames(e, names(e), paste0("Elicit_", names(e)))
      for (nm in names(e)) std[[nm]][i] <- e[[nm]]
    }
  }
}
